...
export default function BrokerSyncSection({ broker: brokerProp, compact = false }: Props) {
  ...
  return (
    <div
      ...
      className={`flex flex-col gap-2 p-3 rounded-xl border bg-zinc-950/50 backdrop-blur-sm ${fresh ? (missingApiKey ? "border-amber-500/30" : "border-sky-500/20") : expired ? "border-amber-500/20" : "border-zinc-800"}`}
    >
      {/* ── Main status row ──────────────────────────────────────────────── */}
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            {fresh ? (
              <div className={`h-8 w-8 rounded-full flex items-center justify-center border ${missingApiKey ? "bg-amber-500/10 border-amber-500/20" : "bg-sky-500/10 border-sky-500/20"}`}>
                {missingApiKey ? <Link2Off className="h-4 w-4 text-amber-400" /> : <Link2 className="h-4 w-4 text-sky-400" />}
              </div>
            ) : expired ? (
            ...
            <div>
              <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                Broker Sync
                {fresh && !missingApiKey ? (
                  <Badge className="bg-sky-500/10 text-sky-400 border-sky-500/30 text-[9px] px-1.5 py-0">Connected</Badge>
                ) : fresh && missingApiKey ? (
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            {brokerInfo.oauthSupported ? (
              ...
              <>
                {!showPaste ? (
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto bg-zinc-900 p-1 rounded-lg border border-zinc-700">
                    <Input value={manualToken} onChange={(e) => setManualToken(e.target.value)} placeholder="Paste your access token here…"
                        ...
                        autoFocus
                      />
                    <Button type="button" onClick={handleSaveToken} disabled={saving} size="sm" className="h-7 px-3 bg-sky-500 hover:bg-sky-400 text-black text-xs font-bold">
                ...
                {brokerKey && !showPaste && (
                  <a href={brokerInfo.portal} target="_blank" rel="noopener noreferrer" className="hidden sm:flex items-center justify-center gap-1.5 h-9 px-3 rounded-md bg-zinc-900 border border-zinc-800 text-[11px] text-zinc-400 hover:text-sky-400 hover:border-sky-500/30 transition-colors">
              ...
              {!showKeyInput ? (
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Button
                    ...
                    size="sm"
                    disabled={savingKey}
                    className="h-7 px-3 bg-sky-500 hover:bg-sky-400 text-black text-xs font-bold"
