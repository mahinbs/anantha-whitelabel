...
export function LiveEntryTrackingSection({
  ...
  return (
    <div className="rounded-lg border border-sky-500/25 bg-sky-500/5 p-3 space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-2 min-w-0">
          <Bell className="h-4 w-4 text-sky-400 shrink-0 mt-0.5" />
          <div>
            <p className="text-xs font-semibold text-white">Entry point alarms</p>
            <p className="text-[11px] text-zinc-400 mt-0.5">
              Daily digest for <span className="font-mono text-sky-200/90">{normalized}</span>. Up to {MAX_TRACKED} symbols total.
              Past digests below.
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {saving ? <Loader2 className="h-4 w-4 animate-spin text-sky-400" /> : null}
          <Switch
        ...
        <div className="border border-white/10 rounded-md bg-black/30 p-2 space-y-2">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <Button
              ...
              variant="outline"
              size="sm"
              className="h-7 text-[10px] px-2 gap-1 border-sky-500/40 text-sky-300"
          ...
          {loading ? (
            ...
            <ul className="space-y-1.5 max-h-40 overflow-y-auto">
              {rows.map((r) => (
                ...
                >
                  <span className="font-mono text-sky-200/90 truncate">{r.symbol}</span>
