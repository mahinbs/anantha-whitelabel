                        ...
                        {hasAlgoAccess && (
                          <div className="space-y-2">
                            <div className="flex flex-wrap items-center gap-2">
                              <Badge className="bg-sky-500/20 text-sky-300 border border-sky-500/40">
                                Subscription Active
                              </Badge>
                              {isInGracePeriod && (
              ...
              <div
                key={plan.id}
                    className={`p-6 rounded-2xl flex flex-col relative transition-all border ${
                      plan.recommended
                        ? "bg-gradient-to-b from-sky-950/40 to-black border-sky-500/30 shadow-[0_0_30px_rgba(20,184,166,0.1)] lg:-mt-2"
                        : "bg-black border-zinc-800 shadow-md"
                    } ${plan.id === "professionalPlan" && "md:col-span-2 lg:col-span-1"}`}
              >
                {plan.recommended && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-sky-500 text-black text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest z-10">
                    Recommended
                  </div>
                )}
                    <h3
                      className={`text-lg font-bold mb-2 ${plan.recommended ? "text-sky-400" : "text-zinc-200"}`}
                    >
                      {plan.name}
                    </h3>
                ...
                <ul className="space-y-3 mb-8 flex-1 text-sm text-zinc-300">
                  {plan.features.slice(0, 6).map((feature, i) => (
                    <li key={i} className="flex gap-3 items-start text-xs">
                          <CheckCircle
                            className={`h-4 w-4 shrink-0 mt-0.5 ${plan.recommended ? "text-sky-400" : "text-sky-500"}`}
                          />
                ...
                <Button
                      className={`w-full py-5 rounded-xl ${plan.recommended ? "bg-sky-500 hover:bg-sky-400 text-black shadow-lg shadow-sky-500/20" : "bg-zinc-100 hover:bg-zinc-300 text-black"} font-bold transition-all`}
                  onClick={async () => {
                        const {
                          data: { session },
                        } = await supabase.auth.getSession();
