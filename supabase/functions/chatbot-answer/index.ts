 ...
 * Also handles platform (Infiniate) support questions.
...
function isJailbreakAttempt(msg) {
...
function isFinanceOrPlatformQuery(msg) {
  const lower = msg.toLowerCase();
...
function detectIntent(msg) {
  const lower = msg.toLowerCase();
  if (/\b(price|trading at|current price|what.?s.*worth|how much is|quote|rate|worth now|value)\b/.test(lower)) return "price";
  ...
  if (m) {
    ...
    if (!/^(I|A|AN|THE|IS|IT|IN|TO|DO|BE|ON|AT|BY|OR|AND|NOT|YES|NO|SO|IF|OK)$/.test(t)) {
      ...
      if ([
        ...
        "BNB",
        "ADA"
      ].includes(t)) return `${t}-USD.CC`;
      return `${t}.US`;
    }
  }
  return null;
}
function inferSymbolFromHistory(history) {
...
function hasFinanceContextInHistory(history) {
...
async function fetchEODHDHistory(symbol) {
  try {
...
function cleanForMarketAux(sym) {
  return sym.replace(/\.(US|BSE|NSE|CC|FOREX|INDX|COMM)$/i, "").replace(/-USD$/, "").replace(/-/g, "");
}
...
function fmt(n) {
  ...
  return v >= 1000 ? v.toLocaleString("en-US", {
...
function enforceRiskLine(answer, intent, msg) {
  ...
  const withoutOldRisk = answer.replace(/\n?\s*⚠️[^.\n]*financial advice[^.\n]*(\.)?/gi, "").trim();
...
function buildPriceBlock(eod, altQuote, sym) {
  ...
  const chgP = data?.change_p ?? data?.changePct;
  const open = data?.open;
  const high = data?.high;
  const low = data?.low;
  const vol = data?.volume;
  const source = eod ? "EODHD" : altQuote?.source === "yahoo" ? "Yahoo Finance" : "fallback";
...
function buildNewsBlock(news) {
  ...
  const lines = news.slice(0, 5).map((n, i)=>{
    const entities = Array.isArray(n.entities) ? n.entities : [];
...
Infiniate is an AI-powered trading intelligence platform.
...
function buildPrompt(opts) {
  ...
  const intentGuidance = {
    ...
    platform: `User is asking about the Infiniate platform or its strategies. Answer using the PLATFORM KNOWLEDGE and USER STRATEGIES provided. Be helpful and specific — list the strategies they have created if asked.`,
  ...
  return `You are a sharp, friendly market intelligence assistant for the Infiniate platform. You speak like a knowledgeable trader friend, not a corporate chatbot. You're direct, insightful, and human. You use real data (prices, news sentiment) to back up what you say.
...
SCOPE RULE (CRITICAL):
You ONLY answer questions about finance, markets, stocks, crypto, commodities, forex, economics, trading strategies, and the Infiniate platform. If the user asks about anything unrelated (e.g. cooking, movies, sports, coding, relationships, weather, general knowledge), politely decline: "I'm a financial market assistant, so I can only help with stocks, crypto, market news, trading, and our Infiniate platform. Try asking me about a stock price or market analysis!"
...
serve(async (req)=>{
  ...
  try {
    ...
    if (isJailbreakAttempt(message)) {
      return new Response(JSON.stringify({
        answer: "I can only help with financial market analysis and the Infiniate platform. Ask me about stock prices, market sentiment, or buy/sell/hold analysis.",
    ...
    const shouldBlockOutOfScope = mode !== "platform" && !isFinanceOrPlatformQuery(message) && (!hasContext || isClearlyOutOfScope(message));
    ...
    if (shouldBlockOutOfScope) {
      return new Response(JSON.stringify({
        answer: "I'm a financial market assistant, so I can only help with stocks, crypto, market news, trading, and the Infiniate platform. Try asking me about a stock price or market analysis.",
        suggestContact: false
      }), {
        status: 200,
        headers: JSON_HEADERS
    ...
    const intent = mode === "platform" ? "platform" : (followUpContext || hasContext) && detectedIntent === "general" ? followUpPriceHint ? "price" : followUpMoveHint ? "impact" : "analysis" : followUpContext && detectedIntent === "general" && followUpPriceHint ? "price" : detectedIntent;
    ...
    if (intent !== "platform" && symbol) {
      ...
      newsArr = symbol ? await fetchMarketAuxNews(symbol) : await fetchGeneralMarketNews();
    } else if (intent === "general") {
      newsArr = await fetchGeneralMarketNews();
    }
    // ── Build context blocks ───────────────────────────────────────────────
    const priceBlock = intent === "price" || intent === "analysis" || intent === "impact" ? buildPriceBlock(eodPrice, altQuote, symbol ?? "asset") : "";
